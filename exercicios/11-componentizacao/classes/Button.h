...
// A biblioteca Arduino.h é a que é incluida por padrão pela IDE do Arduino, ela
// que contém as definições de `digitalWrite()`, `digitalRead()`, etc.
...
// Valor padrão de debounce, que é o intervalo mínimo entre registrar diferentes
// cliques, 200 ms
...
class Button {
 ...
 public:
  ...
  // cada classe necessita de pelo menos um, que tem a função de inicializar
  // suas variáveis internas
  ...
  // A função loop() deve ser chamada em toda iteração do loop() do Arduino, é
  // nela que o processamento principal da biblioteca
  void loop();
};
