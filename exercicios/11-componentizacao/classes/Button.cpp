 ...
 */
void Button::setCallback(ASSINATURA_CALLBACK) { this->callback = callback; }

/**
 *  Configurar o tempo mínimo entre detectar pressionamentos
 */
void Button::setDebounce(unsigned int millis) { this->debounce = millis; }

void Button::loop() {
  // Caso não haja função de callback para chamar, não é necessário executar
  // processamento
  if (!callback) {
    return;
  }
