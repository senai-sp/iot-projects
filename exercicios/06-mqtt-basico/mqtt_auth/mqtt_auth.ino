...
// Esta função será chamada quando o cliente receber uma mensagem em algum
// tópico em que ele está inscrito
...
void loop() {
  // A biblioteca PubSubClient precisa que este método seja chamado em cada
  // iteração de `loop()` para manter a conexão MQTT e processar mensagens
  // recebidas (via a função callback)
  client.loop();
}
