...
void loop() {
  ...
  // Estrutura de decisão com possibilidade de vários caminhos em cima de um
  // único valor também pode ser implementado através de `if`s
  ...
  switch (state) {
    case STATE_REQUEST:
      ...
      if (client.connect(server, 8080)) {
        ...
        // ../projects/aulas/05-ethernet-temperatura.md#formato-da-mensagem-http-post
        // Postamos para a raiz do servidor ('/') conforme documentação do
        // exercício
        ...
        // O cabeçalho Content-Type especifica o tipo de conteúdo de nossa
        // mensagem, neste caso é um texto no formato JSON outros valores
        // possíveis seriam: text/plain: texto qualquer text/html: página HTML
        // etc.
        ...
        // Content-Length deve conter o tamanho do conteúdo da mensagem em
        // bytes, por sorte cada caracter ASCII tem 1 byte, então podemos
        // utilizar String::length()
        client.print("Content-Length: ");
        client.println(json.length());

        // Este cabeçalho especifica que o servidor deve terminar a conexão após
        ...
        // O método flush() força o envio de todo o texto acima através da placa
        // Ethernet caso não o utilizemos, é possível que nossa mensagem fique
        // 'presa' na memória da própria placa e não seja enviada
        client.flush();
    ...
    case STATE_RESPONSE:
      ...
      if (client.available()) {
        ...
        // esta linha não é necessária devido a `state` já ser igual a
        // STATE_RESPONSE, mas torna a intenção explícita
        state = STATE_RESPONSE;
      }
      // Se o servidor desconectar (conforme pedimos pelo header Connection, do
      // HTTP) paramos o cliente e vamos para o modod de espera, para pausar o
      // arduino até o próximo envio de temperatura
      if (!client.connected()) {
        Serial.println();
